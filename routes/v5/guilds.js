...
router.get('/:guild', function(req, res) {
  ...
  let welcomeSys = db.get(`WelcomeChannel_${req.params.guild}`)
  let autoroleSys = db.get(`AutoroleRole_${req.params.guild}`)
  ...
  let decodedIcon = decodeURIComponent(img)

  res.json({ name: nm, image: decodedIcon, botmode: botmode, systems: { inviteblocker: on2, captcha: on, welcomer: on3, autorole: on4, logs: on5, sugestions: on6 } })
});
