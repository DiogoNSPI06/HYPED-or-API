...
router.get('/', function(req, res) {
  let authorization = db.get(`Authorization_${req.query.uuid}_${req.query.authorization}_MailConfirm`)
  
  if(!authorization) {
    return res.json({ message: "103" })
  } 
  let userID = db.get(`HPD_User_Nickname_${req.query.uuid}`)
  if(!userID) {
    return res.json({ message: "102" })
  }

  let UID = req.query.uuid
  
  let Email = db.get(`HPD_User_Email_${UID}`)
  let code = db.get(`HPD_Mail_Code_${UID}`)

  SendMail(Email, code)
  
  db.delete(`Authorization_${req.query.uuid}_${req.query.authorization}_MailConfirm`)
  return res.json({ message: "104" })
});
